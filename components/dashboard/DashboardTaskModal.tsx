            ...
            <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-xl w-full max-w-4xl p-6 sm:p-8 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="overflow-y-auto">
                    {tasks.length > 0 ? (
                        <table className="w-full text-left">
                            <thead>
                                <tr className="border-b-2 border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-700/50">
                                    <th className="p-3 sm:p-4 text-sm font-semibold text-neutral-600 dark:text-neutral-300 uppercase tracking-wider">Judul</th>
                                    <th className="p-3 sm:p-4 text-sm font-semibold text-neutral-600 dark:text-neutral-300 uppercase tracking-wider">Ditugaskan Kepada</th>
                                    <th className="p-3 sm:p-4 text-sm font-semibold text-neutral-600 dark:text-neutral-300 uppercase tracking-wider">Due Date</th>
                            ...
                            <tbody>
                                {tasks.map(task => (
                                    <tr key={task.id} className="border-b border-neutral-200 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-700/50 transition-colors">
                                        <td className="p-3 sm:p-4 font-medium text-neutral-800 dark:text-neutral-100">{task.title}</td>
                                        <td className="p-3 sm:p-4 text-neutral-600 dark:text-neutral-300">{getUserName(task.assignedTo)}</td>
                                        <td className="p-3 sm:p-4 text-neutral-600 dark:text-neutral-300">{new Date(task.dueDate).toLocaleDateString('id-ID')}</td>
                                        <td className="p-3 sm:p-4">
                                            <span className={`px-3 py-1 text-xs font-semibold rounded-full ${statusClass[task.status] || 'bg-neutral-200 text-neutral-800'}`}>
                                                {task.status}
                                            </span>
                                        </td>
                                    </tr>
                        ...
                        <p className="text-center text-neutral-500 dark:text-neutral-400 py-10">Tidak ada pekerjaan dalam kategori ini.</p>
                    )}
                </div>
            </div>
        </div>
